	...
	private function processTag(tag:ITag):Void
	{
		...
		switch (cast(tag.type, Int))
		{
			...
			case TagDefineVideoStream.TYPE:
				processVideoStreamTag(cast tag, currentTagIndex);
		}
	}

		private function processVideoStreamTag(tag:IDefinitionTag, currentTagIndex:Int):Void
	{
		...
		var tagFrameLabel:TagFrameLabel = cast tag;
			...
			currentFrame.labels.push(tagFrameLabel.toString());
			...
			a.push(tagFrameLabel.toString());
			frameIndexes.set(tagFrameLabel.toString(), currentFrame.frameNumber + 1);
